...
namespace LeetCode_Problems.Hard_Problems
{
    ...
    internal class Q502
    {
        ...
        {
            // not finished and it is incorrect

            List<int> maxCapital = new List<int>();
            maxCapital.Add(w);
            ...
            {
                if (initalCap == capital[i])
                {
                    maxCapital.Add(profits[i]);
                }
                else if (maxCapital.Sum() <= capital[i])
                {
                    if (profits[i] > tempProfit)
                    {
            ...
            maxCapital.Add(tempProfit);

            return maxCapital.Sum();
        }
    }
}
